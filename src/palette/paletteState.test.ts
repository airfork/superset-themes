...
describe("paletteReducer", () => {
  it("opens with a cleared query and reset focus", () => {
    const dirty = { open: false, query: "rose", focusedIndex: 4 };
  ...
  it("moves focus down, clamped to the last result", () => {
    const state = { open: true, query: "", focusedIndex: 0 };
    const once = paletteReducer(state, { type: "move", direction: "down", count: 3 });
  ...
  it("moves focus up, clamped to zero", () => {
    const state = { open: true, query: "", focusedIndex: 0 };
    expect(paletteReducer(state, { type: "move", direction: "up", count: 3 }).focusedIndex).toBe(0);
  });

  it("never moves focus past an empty result list", () => {
    const state = { open: true, query: "zzz", focusedIndex: 0 };
  ...
  it("closes on submit", () => {
    const state = { open: true, query: "rose", focusedIndex: 1 };
  ...
  it("returns a new object rather than mutating", () => {
    ...
    expect(INITIAL_PALETTE_STATE).toEqual({ open: false, query: "", focusedIndex: 0 });
  });
});
