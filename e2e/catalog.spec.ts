...
// Phase 3 (Task 10) replaces the legacy catalog filter/grid UI with the LayoutShell + placeholder
// rail/pane. The assertions below target controls and articles that no longer render. Phase 5
// Task 18 rewrites this spec around the new rail-row → ?theme= flow.
test.describe
  .skip("legacy catalog filter UI (revived in Phase 5 Task 18)", () => {
    ...
    test("opens a catalog card detail route with preview tabs and JSON actions", async ({
      page,
    }) => {
      await page.goto("/");
